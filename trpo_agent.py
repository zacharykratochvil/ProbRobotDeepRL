...
class TRPOAgent:
    ...
    def __init__(self, policy, discount=0.98, kl_delta=0.01, cg_iteration=10,
                 cg_dampening=0.001, cg_tolerance=1e-10, cg_state_percent=0.1, env_name=None, env_type="gui"):
        self.policy = policy
        ...
        self.random_save = np.random.randint(0,100000)
        ...
        self.env = SimpleDrivingEnv(env_name, env_type=env_type)
    ...
    def optimize(self):
        ...
        # save
        self.save_model("agent" + str(self.random_save) + ".pth")
    ...
    def train(self, env_name, seed=None, batch_size=12000, iterations=100,
        ...
        for iteration in range(iterations):
            ...
            for i, step in enumerate(range(batch_size)):

                while True:
                    # Take step with agent
                    merged_obs = np.concatenate(observation)
                    flat_obs = np.reshape(merged_obs,[-1])
                    action = self(flat_obs)
